  ...
  describe("Success", () => {
    it("should return 200 with an active seat hold for valid seats", async () => {
      ...
      const res = await request(app)
        ...
        .send({ showId: show.id, showSeatIds: [ss1.id, ss2.id] })
        .expect(200);
  ...
  describe("Validation", () => {
    it("should return 400 when showSeatIds is empty", async () => {
      ...
      const res = await request(app)
        .post(API)
        .set("Authorization", `Bearer ${accessToken}`)
        .send({ showId: crypto.randomUUID(), showSeatIds: [] })
        .expect(400);
    ...
    it("should return 400 when showSeatIds contains duplicates", async () => {
      ...
      const res = await request(app)
        ...
        .send({
          showId: crypto.randomUUID(),
          showSeatIds: [duplicateId, duplicateId],
        })
        .expect(400);
    ...
    it("should return 400 when showSeatIds are not valid UUIDs", async () => {
      ...
      const res = await request(app)
        ...
        .send({
          showId: crypto.randomUUID(),
          showSeatIds: ["not-a-uuid", "also-not-uuid"],
        })
        .expect(400);
    ...
    it("should return 400 when showId is not a valid UUID", async () => {
      const { authUser } = await AuthHelper.getAuthenticatedUser();
      const { accessToken } = authUser.session;
    ...
    it("should return 400 when showId is missing from body", async () => {
      const { authUser } = await AuthHelper.getAuthenticatedUser();
      const { accessToken } = authUser.session;
  ...
  describe("Seat Hold Errors", () => {
    it("should return 400 with SEAT_HOLD_ERROR when a seat is already BOOKED", async () => {
      ...
      const res = await request(app)
        ...
        .send({
          showId: show.id,
          showSeatIds: [availableSeat.id, bookedSeat.id],
        })
        .expect(400);

      expect(res.body.success).toBe(false);
      expect(res.body.error.code).toBe(SeatHoldErrorCode.SEAT_HOLD_ERROR);
    });
  });
});
